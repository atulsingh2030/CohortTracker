...
test('template registry exposes unbounded design freedom and required attribution', () => {
  const template = listTemplateDefinitions().find((item) => item.id === 'template-1');

  assert.equal(template?.designFreedom, 'unbounded');
  assert.equal(template?.requiredAttribution.gitme.url, 'https://github.com/supratikpm/GitMe');
  assert.equal(template?.requiredAttribution.linkedIn.required, true);
  assert.equal(template?.promptSuggestion.includes('the only hard product constraint is visible acknowledgement'), true);
  assert.deepEqual(template?.supportedContextRange, { min: 1, max: 1 });
  assert.equal(template?.updatePolicy.additiveData, 'platform-shell-by-default');
});
